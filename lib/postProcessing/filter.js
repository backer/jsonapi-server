...
filter._filterMatches = function(textToMatch, propertyText) {
  if (textToMatch.slice(0,2) === ">=") {
    textToMatch = textToMatch.substring(2);
    if (typeof propertyText === "number") textToMatch = parseFloat(textToMatch);
  ...
  } else if (typeof propertyText === "number") {
    textToMatch = parseFloat(textToMatch);
    return textToMatch === propertyText;
  } else if (textToMatch === propertyText) {
    return true;
  }
  return false;
};
...
filter._filterKeepObject = function(someObject, filters) {
  for (var filterName in filters) {
    ...
    } else if (someObject.relationships && someObject.relationships.hasOwnProperty(filterName)) {
